  ...
  return queryOne<TimeStats>(`
    SELECT
      ...
      CASE WHEN COALESCE(SUM(turn_count), 0) > 0
        THEN ROUND(1.0 * SUM(total_active_time_ms) / SUM(turn_count), 0)
        ELSE 0
      END AS avg_turn_duration_ms,
      COALESCE(MAX(max_turn_duration_ms), 0) AS max_turn_duration_ms,
      CASE WHEN COALESCE(SUM(response_count), 0) > 0
        THEN ROUND(1.0 * SUM(total_response_time_ms) / SUM(response_count), 0)
        ELSE 0
...
export async function getProjectTimeBreakdown(
  ...
  return queryAll<ProjectTimeBreakdown>(`
    ...
    ORDER BY total_active_time_ms DESC, project_id ASC
    LIMIT 20
  `, range.args.length > 0 ? range.args : undefined);
}
